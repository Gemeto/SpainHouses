...
class FotocasaSpider(scrapy.Spider):
    ...
    def parse(self, response):
        try:
            ...
            if len(response.css("article")) > 0: #TODO clean this code
                ...
                next_page_url = splitUrl[0] + next_page_str + splitUrl[-1].replace(str(next_page_number - 1), "")
    ...
    def parseAnnouncement(self, response, listUrl):
        try:
            ...
            announcement_data["energyConsumption"] = response.css("div.re-DetailEnergyCertificate-itemUnits:nth-of-type(1)::text").get()
